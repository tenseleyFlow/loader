...
class AssistantTurnRequester:
    ...
    async def request_turn(
        ...
        *,
        ...
        filtered_content = self.context.safeguards.filter_complete_content(response.content)
        # Preserve raw content when filtering removes bracket/raw tool syntax so repair can recover it.
        content = filtered_content or response.content
